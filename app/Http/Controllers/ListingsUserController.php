...
class ListingsUserController extends Controller
{
    ...
    { 
        ...
        $keyword = $request->input("search_keyword", "sad");
        $location = $request->input("location");

        if(isset($inputs["location"])){
            // $listings = Listings::SearchByLocation($location)->get();
        }

        // $listings = Listings::where(array('title'=>$keyword,'location_id'=>$location))->get();

        // $listings = Listings::SearchAll($keyword)->get();
        ...
        print_r($location . ":::: ");
        foreach($alllistings as $items){
            print_r($items->address . ":");
        }
         
        return redirect("/testuri?key=" . $keyword);
    ...
    { 
        ...
        return \File::get(public_path() . '/loctest.php');
    ...
    public function keyterm()  
    { 
        
        $keyword = Input::get('key');
        $test = Input::get('test');
        $treatment_snippet = Input::get("slug");
        // $keyword = "kabir";
       // $location = $inputs['location'];

       // $listings = Listings::where(array('title'=>$keyword,'location_id'=>$location))->get();

       $listings = Listings::SearchByKeyword_uri($keyword, 0)->get();
       ...
       if ($keyword == "cavity") {
           ...
           return view('pages.cavity',compact('listings','total_res','keyword','location'));

       }
       if ($keyword == "crown") {

           return view('pages.crown',compact('listings','total_res','keyword','location'));

       }
       if ($keyword == "cyst") {

           return view('pages.cysttumour',compact('listings','total_res','keyword','location'));

       }
       if ($keyword == "deficiency") {

           return view('pages.deficiency',compact('listings','total_res','keyword','location'));

       }
       if ($keyword == "pulpitis") {

        return view('pages.pulpitis',compact('listings','total_res','keyword','location'));

        } 
       if ($keyword == "inflammatory") {

           return view('pages.inflammatory',compact('listings','total_res','keyword','location'));

       }
       if ($keyword == "gingivitis") {

           return view('pages.gingivitis',compact('listings','total_res','keyword','location'));

       }
       if ($keyword == "periodontitis") {

           return view('pages.periodontitis',compact('listings','total_res','keyword','location'));

       } 
       if ($keyword == "cracked") {

           return view('pages.cracked',compact('listings','total_res','keyword','location'));

       }
       if ($keyword == "malocclusion") {

           return view('pages.malocclusion',compact('listings','total_res','keyword','location'));

       }
       if ($keyword == "halitosis") {

           return view('pages.halitosis',compact('listings','total_res','keyword','location'));

       }
       if ($keyword == "infection") {

           return view('pages.infection',compact('listings','total_res','keyword','location'));

       }
       if ($keyword == "depositsdecay") {

           return view('pages.depositsdecay',compact('listings','total_res','keyword','location'));

       }
       if ($keyword == "collapsedbite") {

           return view('pages.collapsedbite',compact('listings','total_res','keyword','location'));

       }
       if ($keyword == "malignancy") {

           return view('pages.malignancy',compact('listings','total_res','keyword','location'));

       }
       if ($keyword == "tonsil") {

           return view('pages.tonsil',compact('listings','total_res','keyword','location'));

       }
       if ($keyword == "abscess") {

           return view('pages.abscess',compact('listings','total_res','keyword','location'));

       }
       if ($keyword == "necrosis") {

           return view('pages.necrosis',compact('listings','total_res','keyword','location'));

       }
       else if ($test) {
          return view("pages.treatment", compact('listings','total_res','keyword','location', 'test', 'treatment_snippet'));
       }
       else { 
    ...
    public function search_listings_keyterm()    
    { 
    ...
    public function submit_listing(){ 
         ...
         if(!Auth::check())
       {
        ...
        $subcategories = SubCategories::where('cat_id')->orderBy('sub_category_name')->get();


        $locations = Location::orderBy('location_name')->get();

        return view('pages.addeditlisting',compact('categories', 'subcategories', 'locations'));
    }
    ...
    { 
        ...
        $i = 0;
            for ($i=0; $i < 5; $i++) { 
                if (isset($_POST['keywords_array'][$i])) {
                    $listings->keywords .= ', ' . $inputs['keywords_array'][$i] . ',';
                    $listings->treatments .= $inputs['checklist'][$c] . " , ";
                    $i += 0;
                }
            }
        ...
        if($current_membership == 'Exclusive')
        {
...
2000);
          ...
          return view('pages.addeditlisting',compact('listing','categories','subcategories','locations','listing_gallery_images', 'services', 'price_range', 'service_range'));
